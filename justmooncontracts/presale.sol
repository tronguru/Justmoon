...
contract JustMoonSale is Ownable {
    ...
    function purchase(address upline) external payable canSale {
        ...
        require(_val >= _currentprice*100000000, "JustMoonSale: error min 100M buy token");
        uint256 _amount = _val*1e10/_currentprice;
        buyers[_to].purchases += _amount;
        totalpurchases += _amount;
        if(totalpurchases >= 10e13*(airdropcycle+1)){
            airdropcycle = airdrops.push(10e12/totalbuyers);
        }
    ...
    function airdrop() external {
        address _to = msgSender();
        User storage u = buyers[_to];
        require(u.purchases > 0, "JustMoonSale: first need to buy tokens");
        require(u.airdropcycle < airdropcycle,'JustMoonSale: airdrop is not available yet');
    ...
    function getCurrentPrice() public view returns(uint256 price) {
        return 1 + 1 * 28 * getCurrentPeriod() / 10000;
    }
    ...
    function getAirdrop(uint256 indx) external view returns(uint256 _airdrops){
        require(indx < airdropcycle,"JustMoonSale: This airdrop hasn't happened yet");
    ...
    modifier canSale() {
        require(salesEnabled == true, "JustMoonSale: token sale closed");
        _;
    }
}
